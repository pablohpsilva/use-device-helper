...
import { useDeviceBreakpoints } from "./hook";
import { DeviceBreakpointProvider } from "./provider";
import "./story.css";

const Component = () => {
  const value = useDeviceBreakpoints(
    "none",
    [
      ["micro", "is micro"],
      ["mobile", "is mobile"],
  ...
  {macos: {chrome: '>80'}})`}
      ...
      <p>
        ...
        <br />
        <span>
          Is Chrome MacOS({`{ macos: { chrome: ">80" } }`})?{" "}
          {value?.satisfies ? "true" : "false"}
        </span>
      </p>
    </div>
  );
...
export default {
  ...
  component: Component,
  decorators: [
    (fn) => <DeviceBreakpointProvider>{fn()}</DeviceBreakpointProvider>,
  ],
};
